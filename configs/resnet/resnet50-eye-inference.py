...
def build_class_to_idx(image_dir):
    ...
    class_names = sorted(path.name for path in image_dir.iterdir()
                         if path.is_dir())
...
def main():
    ...
    model_name = 'resnet50-eye'
    default_checkpoint = str(repo_root / 'work_dirs' / 'resnet50-eye' /
                             'epoch_100.pth')
    default_image_dir = str(repo_root / 'data' / 'eye_area' / 'test')

    parser = ArgumentParser(
        description='Run ResNet50 eye classification inference on one or '
        'more images.')
    ...
    parser.add_argument(
        '--model',
        default=str(script_dir / 'resnet50-eye.py'),
        help='Model config path.')
    parser.add_argument(
        '--checkpoint',
        default=default_checkpoint,
        help='Checkpoint path. Defaults to work_dirs/resnet50-eye/epoch_100.pth.'
    )
    parser.add_argument(
        '--device',
        default='cuda:0',
    ...
    if pred_scores is not None:
        ...
        if len(torch.unique(gt_labels)) < 2:
            ...
            metrics = evaluate_with_mmpretrain(
                pred_scores,
                gt_labels,
                args.roc_path,
                with_auc=False,
            ...
            metrics = evaluate_with_mmpretrain(
                ...
                smooth_roc_points=args.smooth_roc_points)
            add_binary_metrics(metrics, class_names, args.positive_class)
            if not args.no_ci:
                add_confidence_intervals(
                    metrics,
